...
print("=== ENTRAÎNEMENT DU MODÈLE OVER/UNDER BUTS ===\n")

# Charger les features d'entraînement existants
df = pd.read_csv('training_features.csv')

# Créer la target pour Over/Under (seuil ajusté pour FIFA)
# La moyenne est de 9.07 buts, donc on utilise un seuil plus élevé
threshold = 6.5
df['overunder_target'] = (df['avg_total_goals'] > threshold).astype(int)  # 1 = Over, 0 = Under

print(f"Distribution Over/Under (seuil {threshold}):")
print(f"Over (>{threshold}): {df['overunder_target'].sum()} ({df['overunder_target'].sum()/len(df)*100:.1f}%)")
print(f"Under (<={threshold}): {(df['overunder_target']==0).sum()} ({(df['overunder_target']==0).sum()/len(df)*100:.1f}%)")
print()
...
print(f"              Prédit Under    Prédit Over")
print(f"Réel Under    {cm[0][0]:<15} {cm[0][1]:<15}")
print(f"Réel Over     {cm[1][0]:<15} {cm[1][1]:<15}")
print()
...
print("  - overunder_model.pkl")
print("  - overunder_feature_columns.pkl")
print()
